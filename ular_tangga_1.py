...
class UlarTangga:
    ...
    def process_move(self):
        ...
        if not p.is_ai:
            dist, path = bfs_shortest_path(p.pos, self.snakes, self.ladders)
        ...
        if self.state == "game" and not self.dice_rolling:
            ...
            if p.is_ai:
                ...
                if self.ai_think_timer <= 0:
                    # AI pakai BFS untuk pilih — tapi tetap roll dadu random
                    self.roll_dice()
